...
export default function DupaBreakdown({
  ...
  userData, // ✨ Add this line to accept the prop
}) {
  const [localMeasurement, setLocalMeasurement] = useState(measurement);
  const [editing, setEditing] = useState(false);

  // ✨ ADD THIS USE EFFECT HOOK ✨
  useEffect(() => {
    // This will run whenever the `measurement` prop changes
    setLocalMeasurement(measurement);
  }, [measurement]); // The dependency array ensures it only runs when `measurement` changes
      ...
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/reports/update/${reportId}`,
        {
          ...
          body: JSON.stringify({
            ...
            userId: userData._id, // include here
          }),
        }
      );
