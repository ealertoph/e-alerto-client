...
export default function AccountInfo() {
  ...
  // → Effect #1: populate form values when userData arrives
  useEffect(() => {
    if (userData) {
      setFormData({
  ...
  // → Effect #2: load profile picture URL whenever the ID changes
  useEffect(() => {
    if (userData?.profilePic) {
      // point at your new GET endpoint (see server setup)
      setProfilePic(`${backendUrl}/api/user/image/${userData.profilePic}`);
    } else {
      setProfilePic(null);
    }
  }, [userData?.profilePic, backendUrl]);
  ...
  const handleUploadProfilePic = async (e) => {
    ...
    try {
      const res = await fetch(
        `${backendUrl}/api/user/upload-pic/${userData._id}`,
        {
          method: "POST",
          credentials: "include",
          body: formData,
      ...
      setProfilePic(`${backendUrl}/api/user/image/${data.fileId}`);
      setUserData((prev) => ({
        ...prev,
        profilePic: data.fileId, // ✅ update global context
      }));
        ...
        <Box sx={{ height: "100%" }}>
          {!editing ? (
            ...
            <TextField
              ...
              error={!isPhoneValid}
              helperText={
                !isPhoneValid ? "Must be 10 digits starting at 9" : ""
              }
              InputProps={{
                startAdornment: (
